...
// already cleared, and may never be marked applied. `se_gate` is the larger of
// its own sample error and the physical floor; coordinated agreement cannot
// claim precision below a measurement's irreducible uncertainty.
//
...
export type GateInput = {
  ...
  readonly b: number;
  /** The key's sample-only standard error before G3's physical floor. */
  readonly se: number;
  /** The claim's single-sample physical uncertainty; defaults to height for the legacy height-only call surface. */
  readonly sigma_eff?: number;
};

/** The gate's verdict: whether the key may be marked applied, and why (or why not). */
export type GateVerdict = { readonly applied: boolean; readonly reason: string; readonly se: number };

export function gateCorrection(input: GateInput): GateVerdict {
  const se = gateStandardError(input.se, input.sigma_eff ?? SIGMA_EFF.height.value, input.n);
