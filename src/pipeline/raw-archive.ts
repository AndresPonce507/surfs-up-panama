 ...
 *
 * raw/<provider>/dt=YYYY-MM-DD/<HH>/spot=<spot_id>/run=<capture>/execution=<id>.json.gz
 *
 ...
 */
export function rawArchiveKey(provider: string, spotId: string, runAt: Date, executionId: string): string {
  assertSegment('provider', provider);
  assertSegment('spot_id', spotId);
  assertSegment('execution_id', executionId);
  ...
  return `raw/${provider}/dt=${date}/${hour}/spot=${spotId}/run=${run}/execution=${executionId}.json.gz`;
}
...
export function rawArchiveRecord(provider: string, spotId: string, runAt: Date, executionId: string, verbatim: string): RawArchiveRecord {
  return { key: rawArchiveKey(provider, spotId, runAt, executionId), verbatim };
}
