...
export type FetchOverrides = {
  ...
  readonly startup_probe?: () => Promise<void>;
};
...
export async function runFetch(overrides: FetchOverrides = {}): Promise<IngestOutcome> {
  ...
  const outcome = await runIngestOnce({
    ...
    ...(overrides.execution_id === undefined ? {} : { execution_id: overrides.execution_id }),
    ...(startupProbe === undefined ? {} : { startup_probe: startupProbe }),
  });
...
export const handler = async (event: { readonly id?: string } = {}): Promise<{ readonly statusCode: number }> => {
  const outcome = await runFetch(event.id === undefined ? {} : { execution_id: event.id });
