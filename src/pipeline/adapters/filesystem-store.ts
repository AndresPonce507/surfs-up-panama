...
export class FilesystemStore implements IngestStore, BuildStore {
  ...
  async putRaw(record: RawArchiveRecord): Promise<void> {
    await this.write(record.key, encodeText(record.key, record.verbatim));
  }
  ...
  private async readGzip(key: string): Promise<string | null> {
    try {
      ...
      // Pre-existing checked-in captures used a .gz suffix without gzip
      // bytes. Preserve their replay value while all new archives are real
      // gzip objects.
