...
Feature: Today's call, computed from this morning's models
  ...
  @slice-01 @driving_port @in-memory @negative @error @contract-shape:bounded-change @covers-R3
  ...
  @slice-01 @driving_port @in-memory @contract-shape:bounded-change @covers-R1 @covers-R5 @covers-R8
  Scenario: The published call is the physics of the blended members
    ...
    And the call records four used models and zero absent models
    And the call names size as what held the day back
    And the published bundle carries the Spanish call a surfer reads

  @slice-01 @driving_port @in-memory @negative @contract-shape:bounded-change @covers-R6
  ...
  @slice-01 @driving_port @in-memory @negative @error @contract-shape:bounded-change @covers-R9
  ...
  @slice-01 @driving_port @in-memory @negative @error @contract-shape:bounded-change @covers-R10
  Scenario: A dead wind source is named, never papered over
    ...
    And the missing-wind confidence is capped honestly

  @slice-01 @driving_port @in-memory @negative @error @contract-shape:bounded-change @covers-R10
  Scenario: A dead tide source is named, never papered over
    Given the tide source went dark this morning
    And the hourly ingest run completes
    When the build publishes the morning call
    Then the spot still gets a score, from swell and wind alone
    And the record shows tide as absent, not as a number
    And the missing-tide confidence is capped honestly

  @slice-01 @driving_port @in-memory @negative @error @contract-shape:bounded-change @covers-R11
  ...
  @slice-01 @driving_port @in-memory @negative @contract-shape:bounded-change @covers-R8 @covers-R9
  Scenario: One usable wave member still yields an honestly low-confidence call
    Given only wave model "ncep_gfswave016" is usable this morning
    And the hourly ingest run completes
    When the build publishes the morning call
    Then the spot still gets a score from the one usable model
    And the call records one used model and three absent models
    And the single-member confidence is capped honestly

  @slice-01 @driving_port @in-memory @negative @error @contract-shape:bounded-change @covers-R9
  Scenario Outline: A failed forecast source preserves yesterday instead of inventing a call
    Given yesterday's dawn build published a call
    And the forecast provider returned "<failure>" this morning
    And the hourly ingest run completes
    When the build attempts to publish
    Then it refuses to publish
    And yesterday's published call keeps serving, untouched
