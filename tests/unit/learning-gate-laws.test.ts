...
const someB = fc.double({
  min: -2,
  max: 2,
  noNaN: true,
  noDefaultInfinity: true,
});
const someSe = fc.double({
  min: 0,
  max: 2,
  noNaN: true,
  noDefaultInfinity: true,
      ...
      fc.property(
        ...
        (n, reporters, b, se) => {
          ...
          if (verdict.applied) {
            assert.ok(
              n >= G1_MIN_MORNINGS,
              `applied: true must never happen with n (${n}) below G1's floor (${G1_MIN_MORNINGS})`,
            );
            assert.ok(
    ...
    fc.assert(
      fc.property(
        ...
        (rawDifference, sampleCount) => {
          ...
          const storedDifference = shrinkTowardParent(
            rawDifference,
            sampleCount,
            TAU_SPOT_PRIOR,
            parentMean,
          ...
          assert.ok(
            Math.abs(storedDifference) <=
              Math.abs(rawDifference) + Number.EPSILON,
          ...
          assert.equal(
            storedDifference,
            rawDifference,
            "one-spot pooling must retain the spot mean exactly because its parent is the same mean",
          );
        },
      ),
      { numRuns: 100 },
...
describe("gateCorrection: this step's own acceptance numbers, as fixture examples", () => {
  it("refuses twelve mornings from three people, and the reason names the reporters, not the mornings", () => {
    ...
    assert.equal(
      verdict.applied,
      false,
      "twelve mornings clears G1, but three distinct reporters must still refuse it at G2",
    );
    assert.equal(
      verdict.reason,
      "reporters_lt_5",
      "the refusal reason must point at the reporter count, not the morning count G1 already cleared",
    );
  });
  ...
  it("turns on exactly at the boundary: four reporters still refuses, five (with the same ten mornings) applies", () => {
    ...
    assert.equal(
      oneShort.applied,
      false,
      "four distinct reporters is one short of G2, however comfortably G1 is cleared",
    );
    ...
    assert.equal(
      atBoundary.applied,
      true,
      "ten mornings and five distinct reporters is the minimum evidence this step owes a pass on",
    );
  });
  ...
  it("turns on exactly at the significance boundary: a difference at twice se still refuses, a hair beyond it applies", () => {
    ...
    assert.equal(
      atBoundary.applied,
      false,
      "a difference exactly twice its standard error has not cleared it, only reached it",
    );
    ...
    assert.equal(
      justBeyond.applied,
      true,
      "a difference a hair beyond twice its standard error has cleared it",
    );
  });
});
