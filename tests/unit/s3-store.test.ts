...
describe('S3Store (IngestStore + BuildStore over S3)', () => {
  it('writes raw payloads as real gzip bytes under the key as given', async () => {
    ...
    await store.putRaw({ key: 'raw/open-meteo-marine/dt=2026-08-10/06/spot=playa-venao/run=2026-08-10T06-17-00.000Z.json.gz', verbatim: '{"ok":true}' });
    ...
    expect(client.sent[0]?.input.Key).toBe('raw/open-meteo-marine/dt=2026-08-10/06/spot=playa-venao/run=2026-08-10T06-17-00.000Z.json.gz');
