...
describe('S3Store (IngestStore + BuildStore over S3)', () => {
  it('writes raw payloads as a plain, unconditional PutObject under the key as given', async () => {
    const client = fakeClient({});
    const store = new S3Store(client, 'surfs-up-panama-site-602167897909');

    await store.putRaw('raw/open-meteo-marine/dt=2026-08-10/06/payload.json', '{"ok":true}');

    expect(client.sent).toEqual([{
      commandName: 'PutObjectCommand',
      input: { Bucket: 'surfs-up-panama-site-602167897909', Key: 'raw/open-meteo-marine/dt=2026-08-10/06/payload.json', Body: '{"ok":true}' },
    }]);
  });
  ...
  it('reads a prediction back as text', async () => {
    const client = fakeClient({ onGet: () => ({ Body: { transformToString: async () => 'row-one\nrow-two' } }) });
    const store = new S3Store(client, 'bucket');
