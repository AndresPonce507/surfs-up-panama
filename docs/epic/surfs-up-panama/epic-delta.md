...
|---------|-----------------|--------|------------|---------------|
| [F-DAILY-CALL-WITH-PERMANENT-RECEIPTS](../../feature/daily-call-with-permanent-receipts/) | A surfer opens the site at 5:40am and sees today's twenty Pacific spots ranked, with the top one called out and a plain-language reason in Spanish. The next morning, yesterday's numbers are still readable, unchanged, in an append-only archive. | shipped | @walking_skeleton; all 8 slices on main (slices 06/07/08 in `fcc0bb6` / `d9a44b5` / `b835894`; `src/pages/spots/[slug]` and the `@slice-08` page-weight gate verified 2026-08-12) | Keystone per HANDOFF.md section 3. Forecast archives are not retrievable after the fact, so if the prediction log is not written from day one the learning loop becomes permanently impossible. Thin on purpose: one source, physics scoring only (research 09 section 7), learning term wired in and set to zero (research 09 section 7.4). The spot list ships as a seed data file, not code, so nothing is hardcoded to Panama (BRIEF.md constraint 5). Scope per `docs/DISCUSS-decisions.md` 1, 2, 3, 8, 15, 18, 21, 27. |
| [F-BILL-STAYS-ZERO-AND-STAYS-UP](../../feature/f-bill-stays-zero-and-stays-up/) | The hourly ingest stops overnight and Andres hears about it within the hour instead of finding a frozen forecast days later, and the month still closes at $0.00 on the AWS bill. | in-flight |  | Serves BRIEF.md constraint 1. The eleven cost guardrails are research 08 section 10. Their VALUES land with the resources in the keystone row (log retention defaults to never expire, which research 08 section 10.3 calls the number one way free serverless starts costing money). What this row owns as an observable is the dead-man's switch on the hourly ingest, with the budget actions deny policy as the backstop that email alerts alone cannot be. Workspace opened 2026-08-09: 5-slice plan, per-slice charters and DISTILL checklists. Slices 01-03 (CI guardrails) shipped. Verified live 2026-08-09: the $20 billing alarm this row assumes ALREADY EXISTS does not, so slice-03 creates it rather than importing it. **Updated 2026-08-13:** the "slices 04-05 are blocked, zero CloudFormation stacks exist" claim is stale. All four stacks are deployed and have updated repeatedly, so pre-requisites 3 and 4 are satisfied. Slice-04 shipped, met by a real production incident rather than the planned synthetic probe: both dead-man alarms completed ALARM to OK cycles that fired their SNS actions into a confirmed email subscription with zero delivery failures. The substitution and its three limits are recorded in the feature-delta. Slice-05 is still NOT shipped and its row was corrected rather than flipped: `npm run month:close` is on main, but `liveReads()` throws on purpose because Cost Explorer charges per request, so R20's "read from the account" is unmet; its two open halves are a zero-cost project-scoped read design and the console-only `Project` tag activation. The "within the hour" wording above contradicts 08-devops.md section 7, which sets a 2 to 3 hour floor and refuses to tighten it; the charters follow the floor and this row needs Andres to settle the wording. |
...
| [F-SEE-WHAT-KILLED-IT](../../feature/f-see-what-killed-it/) | A surfer taps a spot and sees the one thing that ruined it named outright, for example wind at 0.18, with the size in body-height words and a small map showing which way the break faces. | in-flight | Flipped 2026-08-12: the workspace's own Definition of Done row 9 (Pre-requisite 8) owes this flip; lane building on the deltas branches | `docs/DISCUSS-decisions.md` 17, 18, 20. Naming the weakest sub-score is the entire point of the gate times weighted geometric mean shape in research 09 section 7.3, which exists so a great swell can never average away a ruined wind. |
| [F-PASTE-THE-CALL-INTO-THE-GROUP](../../feature/f-paste-the-call-into-the-group/) | A surfer taps once on a spot page, pastes the day's call and a link straight into the 500-person WhatsApp group, and the link previews with the spot name and its score. | shipped | All 16 roadmap steps carry COMMIT evidence and the paste commits are on main; the acceptance-host repair landed 2026-08-12 (`a0742a4`, riding the integration train) | `docs/DISCUSS-decisions.md` 5, 30. Placed before the report flow on purpose: the WhatsApp group is the distribution channel, so this row brings the audience that the next row then turns into submissions, and submission volume is the binding constraint on every learning feature after that (research 09 section 13.2). Workspace opened 2026-08-09: 5-slice plan, per-slice charters and DISTILL checklists. Not building yet: its share button lives on the spot page, which keystone slice-06 holds open. Builds against the CloudFront hostname per Andres 2026-08-09, so slice-01 sets `site` in astro.config.mjs and a registered domain later is a config change, not rework. One open decision gates slice-04 only: OG image cadence. |
| [F-TELL-US-WHAT-YOU-SAW-COLD](../../feature/f-tell-us-what-you-saw-cold/) | A surfer walking off the beach answers three taps about what they actually saw, and only after that answer is locked in does the site show what it had predicted and how far off it was. | in-flight | Production activation pending: Write-stack Function URLs went live 2026-08-12; the site rebuild with `PUBLIC_REPORT_MINT_URL`/`PUBLIC_REPORT_SUBMIT_URL` and the real browser submit smoke are still owed | `docs/DISCUSS-decisions.md` 4, 9, 11, 28 and the RESOLVED anchoring section. Hard constraint: the label is committed before the reveal screen renders, and the reveal never round-trips back to allow an edit, or the anchoring bias returns and the loop trains on our own prior. Workspace opened 2026-08-09: 5-slice plan, per-slice charters and DISTILL checklists (R1-R45). Not building yet. Slice-01 is a walking skeleton that locks the label with no server and needs zero AWS. Slices 03-05 are deploy-blocked on the Lambda concurrency quota (HANDOFF section 6 item 6, still AccessDenied for andres-cli) and on the missing write stack in infra/lib. Settle the wind enum tokens BEFORE slice-01: a queued offline report replays byte-identical, so a placeholder token becomes a schema-invalid POST later. |
| [F-WORKS-WITH-NO-SIGNAL](../../feature/f-works-with-no-signal/) | A surfer parked at Venao with one bar still sees the last forecast that loaded, and a report filed with no signal at all sends itself once, and only once, when they get back into coverage. | shipped | All five slices verified on main 2026-08-12: `public/sw.js`, the offline queue and flush (`7076afb`), the `queued_duplicate` client half in `src/report/`, and A2HS (manifest route, 192/512 icons, the iPhone hint on the home page) | `docs/DISCUSS-decisions.md` 25, 26. Signal is worst exactly where reports happen, so the queue needs server-side dedup on re-sync rather than client-side trust. |
...
| [F-FORECAST-LEARNS-FROM-THE-BEACH](../../feature/f-forecast-learns-from-the-beach/) | Once enough different people have reported a spot, its forecast visibly moves toward what people actually saw, and where the evidence is still too thin the site refuses to correct it and says so. | in-flight | depends-on F-TELL-US-WHAT-YOU-SAW-COLD; lane mid-flight on `build/f2-learning-01-14-18` (per-reporter offset backfit landed on the lane 2026-08-12), merges after deltas 04-05 per the landing train | `docs/DISCUSS-decisions.md` 22, 24. Non-adjacent dependency: the stage 1 correction is fitted on observations joined to the prediction log. Research 09 section 13.4 gates it at roughly 10 to 30 reports from at least 5 distinct reporters per spot, with shrinkage always on and the applied correction clamped, so a troll or a mis-keyed report cannot produce an absurd public number. The human seed values are never overwritten. |
| [F-TELL-ME-WHEN-ITS-WORTH-THE-DRIVE](../../feature/f-tell-me-when-its-worth-the-drive/) | A surfer who asked about Playa Venao gets one push on the mornings it is worth the drive, and afterwards gets asked how it went, including on the mornings it was bad. | in-flight | depends-on F-TELL-US-WHAT-YOU-SAW-COLD; lane mid-flight on the `build/f2-push-*` branches (steps 01-12..01-19 merge evidence on `build/f2-push-merged`); threshold ruled 2026-08-12: staged, hidden server default 70 now, surfer-facing picker in slice-04 | `docs/DISCUSS-decisions.md` 12, 23. Non-adjacent dependency: the solicited report posts into the same observation record the report flow creates. Soliciting reports regardless of outcome is the primary fix for selection bias in research 09 section 13.5a, which is the hazard most likely to make the learning loop confidently wrong, so the ask-afterwards half is not a nice-to-have bolted onto push. |
| F-READ-IT-IN-YOUR-LANGUAGE | A visitor who does not read Spanish flips one toggle and gets the same call, the same reasons and the same report flow in English, and that choice still holds on the next visit. | pending | DROPPED from scope by Andres 2026-08-12; workspace parked on `build/f2-i18n`; the `/en/` routes from Design 07 remain live | `docs/DISCUSS-decisions.md` 8. Placed last on purpose so one translation pass covers settled copy instead of translating the same strings twice. This is also the seam that proves copy is not bound to one locale, which is what a second country needs later (BRIEF.md constraint 5). |
