...
| `windWord(s_wind, thresholds) -> word` | `clean` if `s_wind >= 0.75`, `bumpy` if `>= 0.40`, else `choppy`; thresholds data, unfit priors | Reuses the physics instead of a second wind formula; the word vocabulary is C2's canonical constants (report labels use the same words, `domain-model.md` §7.3), single source |
