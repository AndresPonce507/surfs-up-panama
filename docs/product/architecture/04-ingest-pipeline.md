...
4. **Durable side effect #1: PUT the verbatim payload batch to `raw/<provider>/dt=<date>/<HH>/spot=<spot_id>/run=<capture_ts>.json.gz`**,
   ...
   instant rendered `YYYY-MM-DDTHH-mm-ss.sssZ`; every raw body is actual gzip
   bytes, never plaintext under a `.gz` suffix.
