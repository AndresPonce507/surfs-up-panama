...
## U8 restraint observation (verbatim from the roadmap quality contract, step 01-10)

Camino el flujo entero a 390 px: en ningún momento, ni en la pantalla uno, ni recargando a mitad del camino, ni en la confirmación, aparece un puntaje, una banda de hoy, una palabra de viento de hoy ni ninguna pista del pronóstico; la confirmación se lee tranquila y sin números.
...
| 2026-08-10 | Vera (nw-user-examiner) | FAIL | Fresh source-blind 01-10 examination. I followed the sole start recipe in `/Users/andres/psb-report`: `npm ci` completed successfully, but `npm run build` exited 1 both before and after that dependency step. Per the charter, no public site can be started or reached at 390 px, so I could not observe the report form, reload, offline flow, confirmation, or Back behavior. A beta user cannot use a feature whose required build recipe does not produce the site. |
| 2026-08-10 | Vera (nw-user-examiner) | PASS | Fresh source-blind retry. The prescribed build completed and the preview at 390 px showed the blank report form with exactly the 7 size, 3 wind and 4 quality choices, with no score or forecast material. After choosing all three and sending, `/spots/playa-venao/reportado/` showed `Reporte guardado` and exactly `Guardado. Cuando vuelva la señal lo mandamos y te decimos cómo nos fue.` with only a Playa Venao return link; the same confirmation rendered after reload. Reloading a partly answered form left zero radios selected. The confirmation return link landed on the spot page and reopening the report showed zero selected radios. With browser network emulation offline, choosing all three and sending reached the identical confirmation, with no error. The 390 px form had no horizontal overflow and appeared complete in the rendered dark theme. |
