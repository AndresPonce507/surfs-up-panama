...
|------|----------|---------|--------------|
| 2026-08-10 | Vera | INDETERMINATE | Built dist/ from HEAD d9bd41e (01-09). Rendered index.html (Hoy tab) shows 20 ranked rows, each with `<details class="confidence">` containing "Confianza baja" or "Confianza media". Only 2 unique reasons exist across all rows (one per confidence level). Each reason ~204–205 characters, derived from Confidence.astro (level alone), not ConfidenceDetail.astro. Examined rows 1–20: no row-specific reason text, no per-spot-per-day phrasing per U8 requirement. Both commits (61d3c15, 7c3a71e) and their own commit messages explicitly state: "ConfidenceDetail.astro already renders nowhere in the built site (no production mount exists yet)." The component is correctly unit-tested (99/99 tests pass, including 01-07 honesty guards), but SpotDetail.astro does not mount it, and per-row reasons have not been composed into the published surface yet. Cannot verify "cada razón se lee como una frase de surfista sobre este spot y este día" against the rendered page because the wiring required to test that requirement (cross-lane diff at 01-11, per commit messages) is not present. |
