...
| slice-02 | The big card reads as glass over water, and on a phone that cannot render glass it still reads perfectly as a solid card. | pending | depends-on slice-01 | `backdrop-filter: blur(12px) saturate(140%)` already ships at `src/styles/components.css:34` and `:266`; it produces nothing today because the surfaces behind it are `#FFFFFF` and `#F2F4F6`. Slice-01 gives it something to blur. This slice is therefore the *fallback*, not the effect: the tokens file already states the rule as "solid-fallback-first everywhere", and `backdrop-filter` carries a real GPU cost on the cheap Android phones this audience uses. The slice is only done when the surface is proven legible with the filter disabled, which is the state a low-end device actually renders. |
