...
| slice-02 | The tide number becomes real for every spot that can honestly reference a tide station, the tide factor rejoins the score, and "confianza alta" becomes reachable exactly where the models genuinely agree tightly, never by a lowered bar. Unmapped spots keep saying the tide is missing. | data-gated | depends-on slice-01; mapping policy accepted 2026-08-13 | The policy is now closed in `adr-tide-source-chain.md`: a mapping needs an auditable exact-spot record of at least 28 high/low events across 14 consecutive local days, p90 phase error at most 30 minutes, no error above 45 minutes, and every daily range ratio within 0.80–1.20. No current spot has that record. Balboa 9812501 and Cristobal 9817583 remain candidates, never coast-wide defaults. The adapter and score path can be built, but activation waits for real validation data; the tide stays dark meanwhile. |
...
| slice-05 | The reason stops comparing models against nothing: once a spot's own spread history exists, "the models split worse than usual for this spot" replaces absolute thresholds, the only form research 09 says carries real signal. | code-ready, data-gated | depends-on slice-01; dormant until 30 completed spot-local days | The source-driven immutable-history reader, pure activation selector, and S3 boundary are ready but deliberately unconnected to the production build. Activation policy is settled and closed (`adr-spread-climatology-activation.md`): 30 distinct completed spot-local days of PublishedCall history. Live S3 has only two date partitions on 2026-08-13, so the selector remains absolute and current public copy stays unchanged. |
