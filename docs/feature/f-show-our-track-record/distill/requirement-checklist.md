...
`red-classification.md`): slice-02's rows are covered and RED, slices 03-05's rows are
...
| R5 | Producer-shape half: `scorecard-refusal-laws.feature` scenario "El contador sale de los dos enteros del bloque y con su forma exacta" (`@covers-R5`, slice-02, RED) | **SPLIT, honestly (updated 2026-08-10).** The slice-01 refusal stands for what it refused: no payload-field assertion without a producer. The shape law (`counter` composed from the block's own integers, any state) became assertable the moment slice-02 owns the block producer, and is covered there. The build-LOUD half on the wire stays with slice-03 (roadmap 03-02), still blocked |
| R6 | Rendered half: slice-01 scenarios 1, 2 and 4 (threshold pinned at exactly 30). Producer half: slice-02 scenario "El contador sale de los dos enteros del bloque y con su forma exacta" (`@covers-R6`, RED) asserts the block carries `REPORTS_REQUIRED` from its one exported home | COVERED across the two slices (updated 2026-08-10); the write-path P3 composer's import of the same constant remains F-TELL's obligation |
...
| R10 | `scorecard-refusal-laws.feature` scenario "Con menos de cinco personas de verdad nunca se publica una cifra" (`@covers-R10`, `@property`) | COVERED, RED (2026-08-10 slice-02 DISTILL) |
| R11 | scenario "Con menos de diez observaciones emparejadas nunca se publica una cifra" (`@covers-R11`, `@property`) | COVERED, RED |
| R12 | scenario "Reportes coordinados que están demasiado de acuerdo nunca aflojan la reja" (`@covers-R12`): the floor law, the se-field-carries-se_gate pin, and coordinated-never-publishes-earlier | COVERED, RED |
| R13 | scenario "El filtro de confianza se ve disparar: una credencial joven pierde sus muestras" (`@covers-R13`): nonzero fixture config drops the young credential; zero config bit-identical to unfiltered | COVERED, RED |
| R14 | scenario "Cada reporte se empareja solo con la hora y la playa que el sitio de verdad pronosticó" (`@covers-R14`): join key, sign convention, land_masked exclusion, unpaired-never-guessed | COVERED, RED |
| R15 | scenarios "El orden en que lleguen los reportes jamás cambia la cuenta" and "Sumar un reporte nuevo no reescribe ningún día ya contado" (`@covers-R15`, `@property`) | COVERED, RED |
| R16 | scenario "Las ventanas de 30 y 90 días derivan el sesgo y el error con las fórmulas asentadas" (`@covers-R16`): formulas plus C5 resolution at read time | COVERED, RED |
| R17 | scenario "El viento no entra al historial por ninguna puerta" (`@covers-R17`, `@negative`): loud rejection naming wind; recompute path re-checked in roadmap 02-07 | COVERED, RED |
| R18 | scenario "Los anclajes de calidad tienen una sola casa y el puntaje los cita tal cual" (`@covers-R18`): anchors from report-vocab only, 25-point floor cited | COVERED, RED |
...
| R20 | scenario "Todo el historial se reconstruye idéntico desde los dos registros inmutables" (`@covers-R20`, `@property`): incremental fold equals batch recompute | COVERED, RED |
