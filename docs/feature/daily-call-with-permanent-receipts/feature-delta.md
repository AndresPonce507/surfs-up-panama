...
| slice-03 | A surfer sees today's twenty Pacific spots ranked best first on the home page, in Spanish, and the order actually changes when the swell does. The spot list is a seed data file: adding a spot is a data edit, not code. | active RED | @distill-active | The launch policy names 20 source-backed Pacific spots and three explicit exclusions in `data/spots/pa-pacific-launch-v1.json`; the RED contract now drives the public home and pipeline surfaces. |
...
| --- | --- | --- | --- |
| DISCUSS Slice Plan slices 01-02 | Slice-01's Venao vertical remains unchanged; Slice-02 protects the irreplaceable archive and concrete cost limits before any deploy. | n/a | Slice-03 is active RED in JIT DISTILL; Slice-04 onward scenarios remain absent. |
...
| DELIVER prerequisite | Slice-01 and Slice-02 shipped; Slice-03 is active RED in JIT DISTILL | Slice-02's fresh delegated independent APPROVED verdict and `SliceCleared` preceded DES commit `592d660`. `des verify-slice-commit` and `des run-contract-gate --verify-gate-scope` passed. Slice-03's launch policy prerequisite closed on 2026-08-09. |
...
Slice-02 shipped in `592d660`. Slice-03 is active RED in JIT DISTILL with its launch policy prerequisite closed.
