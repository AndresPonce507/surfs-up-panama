...
| slice-05 | A surfer flips to Mañana and gets tomorrow's own ranking with tomorrow's own numbers, and the site says plainly that past tomorrow it will not pretend to know. | shipped | @commit-63d5b1e | Verified candidate `63d5b1e` on `slice05-repair-combine`. Full local CI 9/9, 50 acceptance scenarios and 426 steps, E2E, and a fresh independent acceptance review all pass. The permanent source-blind Terra examiner recorded PASS against the hosted preview: `/manana/` returns `VE A Santa Catalina - La Punta`, score 91, reason `Cabeza a un metro más y limpio al amanecer.`. `/pasado-manana` correctly exposes no forecast. The strict two-day validator now rejects a cloned day ranking against both `days[0].spots` and the legacy `current.calls`, so a tomorrow page that merely copies today is refused. |
| slice-06 | A surfer taps any spot and gets that spot's own page: today's and tomorrow's numbers, size in body-height words with metre ranges beside them, and the best window to go. | shipped | @commit-fcc0bb6 | Integrated on main in `fcc0bb6` ("give every spot its own page with today, tomorrow, size and window"); `src/pages/spots/[slug]` renders it. Row flipped 2026-08-12 truth pass. |
| slice-07 | Every ranked row carries a confidence level with the reason one tap away, in plain Spanish: how far the models agree, and whether anyone has confirmed conditions from the beach. The level never claims more certainty than the data earns. | shipped | @commit-d9a44b5 | Integrated on main in `d9a44b5` ("every ranked row carries its confidence level and honest reason - slice-07"), with layout repairs `4f9dc00` and `a093e4c` also on main. Row flipped 2026-08-12 truth pass. |
| slice-08 | The home page loads in under two seconds on a beach 3G connection, and a build that would break that fails CI, naming the route, the measured bytes, and the ceiling. | shipped | @commit-b835894 | Integrated on main in `b835894` ("gate page weight against the beach-3G ceilings"); the `@slice-08` scenarios live in `tests/acceptance/daily-call-with-permanent-receipts/page-weight-budget.feature`. Row flipped 2026-08-12 truth pass. |
