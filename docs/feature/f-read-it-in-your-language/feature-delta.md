...
the exact producer handoff lives in `deliver/01-01-call-en-blocker.md`. The slice remains pending
on implementation and generated-surface evidence, not on copy or schema authority.
...
| 2 | **The English call-line source.** ANSWERED 2026-08-10. The producer emits both locale members from the same row's `size_band`, `wind_state` and `best_window`; pages render and never compute. Exact §10 English: `{Size}, {wind} wind, best from {start} to {end}.`; missing wind becomes `no wind data`; missing window becomes `no estimated window`. The current Spanish branches remain authoritative. The bundle target is the domain model's `call{es,en}`; the current compatibility surface requires `call_es` plus `call_en`; `forecast.ts` maps both into a non-partial locale record. Historical dawn receipts keep their immutable Spanish-only shape, while new receipts naturally retain both. Exact seams, failure rules and coverage matrix: `deliver/01-01-call-en-blocker.md`. Implementation remains producer-lane owned and serial across `build.ts`, bundle/static schemas, validator, adapter and generated surface. | READ-01's call line; READ-05 inherits the same answer | producer lane | ANSWERED 2026-08-10; implementation pending |
