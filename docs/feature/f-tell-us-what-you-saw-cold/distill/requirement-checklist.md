...
| R4 | Ordering is commit, then `history.replaceState`, then render: Back from the confirmation lands on the spot page at every moment, never on an editable form; reopening the report screen starts a blank report with a fresh `report_id` (application-architecture §8 L3). **Reconciled with R26 on 2026-08-13** (`deliver/wave-decisions.md`): R26's page-open flush may send a waiting report on that screen, but may not take the screen away from it. The flush leaves the address, the form and the heading alone, and its result never renders inline on the form screen (slice-01, held jointly with slice-03) | validation |
...
| R26 | This slice owns submit-while-online and the page-open flush trigger only; the `online` event, service worker activation and the backoff ladder are F-WORKS-WITH-NO-SIGNAL's (HANDOFF §7 flush ownership). **Reconciled with R4 on 2026-08-13** (`deliver/wave-decisions.md`): the trigger is kept whole, but a flushed report is acknowledged by a neutral, number-free line in the existing notice surface and its receipt is reachable only by an explicit link. A comparison rendered above a fresh blank form would anchor the next cold capture to our own number, which the RESOLVED anchoring section of `docs/DISCUSS-decisions.md` forbids (slice-03) | validation |
...
| R4 | `@covers-R4`: "Back never returns to an editable form and a new report starts blank" (slice-01, now runs with the page-open flush live) plus the slice-03 negative "A report that sends itself on open puts no reveal on the form screen" | Covered and RECONCILED with R26 on 2026-08-13 (`deliver/wave-decisions.md`). The slice-01 scenario's closing oracle moved from on-phone cardinality to the two identities observed on the wire, because a flushed report is discarded on its matching receipt and two saved rows can no longer coexist once there is signal. R4's promise, a fresh `report_id` per visit, is unchanged. |
...
| R20 | `@covers-R20`: "Sending the same saved report again leaves one arrival" | Mandatory real-handler acceptance evidence: it replays the browser-created durable record and requires the idempotent receipt. Storage cardinality is nonvisual, so U8 observes only one calm arrival surface; deployed replay remains INDETERMINATE until its origin and environment exist. |
...
| R26 | `@covers-R26`: "Opening the report screen sends a report that was already waiting" (external) and "Opening the report screen sends an already saved label" (`@local-real-io`, executable without `REPORT_ACCEPTANCE_ORIGIN`), plus the negative "A report that sends itself on open puts no reveal on the form screen" | RECONCILED with R4 on 2026-08-13 (`deliver/wave-decisions.md`). The trigger is kept whole and is proven locally against the real local write adapter; what moved is where the receipt may appear. The local negative first proves the send happened (accepted answer plus drained queue) so it cannot pass vacuously. The external variant remains RED pending the real report journey; reconnect and service-worker triggers remain out of scope. |
