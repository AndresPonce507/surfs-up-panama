...
| R12 | `@covers-R12`: "The local gate names every protection that keeps a write flood bounded" | Covered; RED (`MISSING_FUNCTIONALITY`): current gate names the historical future-URL deny scope but does not inspect the four URLs' exact-origin public posture. |
| R13 | `@covers-R13`: same source-blind terminal scenario | Covered; RED (`MISSING_FUNCTIONALITY`): current generic Lambda concurrency output has no separate report 2, mint 1, push 1 and photo-presign 1 boundaries. |
| R14 | `@covers-R14`: same source-blind terminal scenario | Covered; RED (`MISSING_FUNCTIONALITY`): current output has no fixed provisioned 25 WCU / 25 RCU write-store check. |
| R15 | `@covers-R15`: same source-blind terminal scenario | Covered; RED (`MISSING_FUNCTIONALITY`): current output has no four-write-breaker inspection. |
| R16 | `@covers-R16`: same source-blind terminal scenario | Covered; RED (`MISSING_FUNCTIONALITY`): current output has no device-only 20 / 10 / 20 quota inspection or per-IP prohibition. |
| R17 | `@covers-R17`: the bounded-flood scenario, unreadable-declaration scenario and one-value safeguard regression scenario | Mixed and honest: the generic missing-declaration error is already PASSING as a regression guard; all eight controlled write-path bites and their diagnostics remain RED until the production write policy is inspected before later CI phases. |
| R18 | `@covers-R18`: bounded-flood terminal scenario | Covered; RED (`MISSING_FUNCTIONALITY`): current gate never identifies `system-architecture.md` §6.1 as the corrected sizing source. |
