...
- **The client-only versus deploy-blocked split, stated exactly.** Buildable now with zero AWS and
  shippable to the built site: slice-02's A2HS hint (0 JS, settled copy) and slice-01's island with
  its designed states, EXCEPT any rendered on-state, which is illegal until a real subscribe
  round-trip exists. Buildable now (pure handler cores, CDK declarations, credential-free synth,
...
| 2 | Every on-state is honest: "listo" renders only after server ack; no notifications-on state anywhere is ever rendered without a real browser `PushSubscription` and a stored `(spot_id, endpoint_hash)` item; return-visit state derives from `PushManager.getSubscription()` plus stored state, never a remembered flag (P6 no-false-green, `application-architecture.md` §7). |
...
| Out | Lands in |
|---|---|
| The service worker file, its registration snippet, the PWA manifest, `/sin-senal`, install mechanics, read cache | F-WORKS-WITH-NO-SIGNAL. This feature consumes the `push` and `notificationclick` handlers through the seam contract below and never edits that lane's files. |
...
| SNS, FCM SDK/topics, SQS fan-out, any third-party push dependency | Never; each explicitly rejected in `adr-push-vapid-direct.md` alternatives. |
...
F-WORKS-WITH-NO-SIGNAL owns the service worker file; its DISCUSS is running in a concurrent lane.
Per the `HANDOFF.md` §7 lesson, the contract is settled here in writing before any parallel
dispatch, so neither lane decides mid-flight something the other depends on. This feature needs
the following implemented in that file and will never edit it:

**Payload** (composed and encrypted by this feature's notify job, aes128gcm, at most 2 KB wire per
`07-write-path.md` §8.5; always valid JSON, producer-guaranteed):
...
Estimated cost in their file: roughly 0.3 KB gz for both handlers; flagged to the SIGNAL lane for
their byte budget. Acceptance of this contract by the SIGNAL lane is Pre-requisite 4.
...
| 4 | **The service worker seam.** The SW file, its registration and the PWA manifest are F-WORKS-WITH-NO-SIGNAL's, whose DISCUSS runs concurrently right now. Owed from them: the `push` and `notificationclick` handlers per the contract section above, SW registration on reading routes, and the `display: standalone` manifest. Owed to them from this settlement: A2HS hint ownership, with the recommendation recorded in slice-02's row (the hint's words promise avisos, so the hint ships HERE; manifest and SW ship THERE). Per the `HANDOFF.md` §7 lesson this must be settled between the two lanes before either dispatches DELIVER work on the seam. | slice-01 (Android needs a registered SW with the push handler for any notification to render) and slice-02 (manifest, installed context) | coordinator, with the SIGNAL lane | open, contract proposed above |
| 5 | **Account Lambda concurrency quota** (`HANDOFF.md` §6 item 6, still AccessDenied for `andres-cli` as of 2026-08-09). RC 1 on push and notify needs the same headroom answer as the report fn; at a quota at or under 102, `PutFunctionConcurrency` is rejected and the breaker design does not exist. | Deploy of slices 01, 03, 04; nothing about the build | Andres (console or a policy grant) | open |
...
| 7 | **Push-service host allowlist contents.** The ADR names the classes (FCM, Apple web push, Mozilla autopush, WNS) but the research corpus contains no verified hostname list, and a wrong list silently locks a real browser out of subscribing; the ADR accepts that gap as self-reporting because the rejection is loud and names the host. The list is a config data file, additive by PR, authored inside slice-01, with each hostname verified against current vendor documentation at authoring time, never written from memory. | Nothing; recorded so the list is verified, not invented | slice-01 DELIVER | open |
...
| No service worker file, no SW registration, no PWA manifest | `public/` contains only `favicon.svg`; no `manifest` reference anywhere in `src/` |
